...
const RENDER_PARKING_INDEX = PIT_WAYPOINTS_COUNT >= 3 ? PIT_WAYPOINTS_COUNT - 3 : -1;
  ...
  if (trackData && trackData.pitWaypoints && trackData.pitWaypoints.length > 1) {
    ...
    trackData.pitWaypoints.forEach((wp, index) => {
      ...
      const dx = (index < trackData.pitWaypoints.length - 1)
        ? trackData.pitWaypoints[index + 1].x * SCALE - wp.x * SCALE
        : wp.x * SCALE - trackData.pitWaypoints[index - 1].x * SCALE;

      const dy = (index < trackData.pitWaypoints.length - 1)
        ? trackData.pitWaypoints[index + 1].y * SCALE - wp.y * SCALE
        : wp.y * SCALE - trackData.pitWaypoints[index - 1].y * SCALE;
    ...
    for (let i = trackData.pitWaypoints.length - 1; i >= 0; i--) {
      ...
      const dx = (i < trackData.pitWaypoints.length - 1)
        ? trackData.pitWaypoints[i + 1].x * SCALE - wp.x * SCALE
        : wp.x * SCALE - trackData.pitWaypoints[i - 1].x * SCALE;

      const dy = (i < trackData.pitWaypoints.length - 1)
        ? trackData.pitWaypoints[i + 1].y * SCALE - wp.y * SCALE
        : wp.y * SCALE - trackData.pitWaypoints[i - 1].y * SCALE;
